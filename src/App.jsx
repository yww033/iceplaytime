...
const apiKey = "AIzaSyBKmszrIeGMlBAoii6TjpNLJeepZP0TD1M"; 
          ...
          <div>
            ...
            {isPresetOpen && (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-6 px-2 animate-in fade-in slide-in-from-top-2 duration-300">
                {presetPhrases.map(p => (
                  <div key={p.id} onClick={() => speak(p.korean)} className="bg-white p-5 rounded-[2.2rem] border border-slate-100 shadow-sm flex items-center justify-between group cursor-pointer hover:border-[#D7897F]/30 hover:shadow-md transition-all active:scale-[0.98] active:bg-[#D7897F]/5">
                    ...
                    <div className="w-12 h-12 shrink-0 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-300 group-hover:text-[#D7897F] group-active:text-[#D7897F] transition-colors shadow-inner"><Volume2 size={22}/></div>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="pt-4 border-t border-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 px-2">
              {survivalPhrases.sort((a,b)=>(a.order||0)-(b.order||0)).map((p, idx) => (
                <div key={p.id} draggable onDragStart={() => setDraggedId(p.id)} onDragOver={e => e.preventDefault()} onDrop={e => handleDrop(e, idx)} onClick={() => speak(p.korean)} className="bg-[#96C7B3] p-5 rounded-[2.2rem] shadow-md flex items-center gap-4 cursor-pointer hover:bg-opacity-90 transition-colors animate-in zoom-in-95 group">
                  <div className="text-white/60 cursor-grab hover:text-white transition-colors"><GripVertical size={20}/></div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                     <button onClick={(e) => { e.stopPropagation(); speak(p.korean); }} className="p-2 text-white hover:text-white/70 transition-colors"><Volume2 size={20}/></button>
                     <button onClick={(e) => { e.stopPropagation(); setSurvivalPhrases(survivalPhrases.filter(item => item.id !== p.id)); }} className="p-2 text-white/60 hover:text-white transition-colors"><Trash2 size={20}/></button>
                  </div>
                </div>
              ))}
  ...
  useEffect(() => {
    const fetchRealWeather = async () => {
      ...
      try {
        ...
        const geoRes = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
...
const SplitBillSection = ({ user }) => {
  ...
  const handleAiParse = async () => {
    ...
    try {
      ...
      setErrorMsg("AI 解析失敗，請確認文字內是否包含明確的金額與墊付人");
    }
...
const CurrencySection = () => {
  ...
  return (
    <div className="p-4 md:p-0 space-y-6 md:space-y-8 animate-in fade-in duration-500 text-slate-800 pb-20">
      
      {/* 匯率資訊卡片 */}
      <div className="bg-white p-5 sm:p-8 md:p-10 rounded-[2.5rem] md:rounded-[3.5rem] shadow-sm border border-slate-200 relative overflow-hidden">
        ...
        {/* 輸入區域 RWD 配置 */}
        <div className="flex flex-col md:flex-row items-center md:items-stretch gap-3 md:gap-8 w-full">
          
          <div className="flex-1 w-full flex flex-col md:grid md:grid-cols-[1fr_auto_1fr] gap-3 md:gap-6 md:items-center">
            
            {/* 韓元 KRW */}
            <div className="bg-slate-50 p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border border-slate-200 w-full flex flex-col justify-center shadow-inner hover:border-[#6398A9]/50 transition-colors">
              <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest ml-1 mb-1 md:mb-2">韓元 KRW</p>
              <div className="flex items-baseline gap-2 md:gap-3">
                <span className="text-2xl md:text-4xl font-serif text-[#6398A9]">₩</span>
                <input type="number" placeholder="0" className="w-full text-3xl md:text-5xl font-serif outline-none bg-transparent placeholder:text-slate-200 text-slate-800" value={krw} onChange={(e) => handleKrwChange(e.target.value)} />
              </div>
            </div>

            {/* 切換圖示/分隔線 & 行動版歸零按鈕 */}
            <div className="flex justify-center items-center gap-4 w-full md:w-auto py-1 md:py-0">
              <div className="w-10 h-10 md:w-14 md:h-14 bg-slate-50 rounded-full flex items-center justify-center text-slate-400 shadow-sm border border-slate-200">
                <ArrowDownUp size={20} className="md:-rotate-90"/>
              </div>
              <button onClick={clearAll} className="md:hidden px-6 h-10 bg-[#F9B95C] text-white rounded-full flex items-center justify-center gap-2 shadow-md active:scale-95 transition-all text-xs font-bold tracking-widest">
                <Eraser size={16} /> 歸零
              </button>
            </div>
            ...
            {/* 台幣 TWD */}
            <div className="bg-slate-50 p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border border-slate-200 w-full flex flex-col justify-center shadow-inner hover:border-[#6398A9]/50 transition-colors">
              <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest ml-1 mb-1 md:mb-2">台幣 TWD</p>
              <div className="flex items-baseline gap-2 md:gap-3">
                <span className="text-2xl md:text-4xl font-serif text-[#6398A9]">$</span>
                <input type="number" placeholder="0" className="w-full text-3xl md:text-5xl font-serif outline-none bg-transparent placeholder:text-slate-200 text-slate-800" value={twd} onChange={(e) => handleTwdChange(e.target.value)} />
              </div>
            </div>
          </div>
          
          {/* 電腦版專用巨型歸零按鈕 (手機版隱藏) */}
          <button onClick={clearAll} className="hidden md:flex w-28 self-stretch bg-[#F9B95C] text-white rounded-[2.5rem] flex-col items-center justify-center gap-3 hover:bg-[#E5A548] active:scale-95 transition-all shadow-md">
            <Eraser size={28} />
            <span className="text-[11px] font-bold tracking-[0.3em] [writing-mode:vertical-lr]">歸零</span>
          </button>
        </div>

        <div className="mt-5 md:mt-8 pt-4 md:pt-6 border-t border-slate-100 flex justify-end">
      ...
      <div className="space-y-3 md:space-y-4">
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2 md:gap-4">
          {quickPickKRW.map(val => (
            <button key={val} onClick={() => handleKrwChange(val.toString())} className="bg-white p-3 md:p-5 rounded-2xl md:rounded-[1.5rem] border border-slate-100 shadow-sm flex flex-col items-center justify-center gap-1.5 hover:border-[#D7897F]/30 hover:shadow-md active:scale-95 transition-all group">
